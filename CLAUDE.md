...
### 수집 파이프라인 phase 구성 (Day 6+)
- **Phase 1 (병렬)**: serve, dabang, peterpanz, daangn, zigbang
  - serve가 naverAtclNo cross-ref 추출 → naver 어댑터에서 중복 스킵
- **Phase 2 (병렬)**: naver + kbland
  - naver는 phase 1의 cross-refs 의존
  - kbland는 phase 1 worker pool 끝 큐 대기 회피 위해 phase 2 배치
- 전체 wall ≈ max(phase1) + max(phase2) ≈ 17 + 40 = ~57분 (베이스라인 78분 → -27%)

### 중요 동작 규칙: deleted_at 자동 회복 (Day 7+)
- `ops_db_persistence.mjs` ON CONFLICT 시 `deleted_at = NULL`로 자동 클리어
- 즉 **재수집된 매물은 자동으로 active 회복** — 잘못 마킹된 매물도 다음 cron에서 복구
- 진짜 expired 매물은 `check_listing_status.mjs`가 다음 단계에서 다시 마킹
...
- **collection_runs**: cron 실행 메타. 컬럼:
  - `mode` (API/STEALTH_AUTOMATION/BLOCKED): 수집 방식
  - `run_mode` (full/incremental): 수집 범위 — Day 1+, 마이그레이션 031
...
bash scripts/collect_wrapper.sh                    # full 모드 (기본)
bash scripts/collect_wrapper.sh incremental        # incremental 모드 (Day 1+)
COLLECT_EXCLUDE_PLATFORMS=naver bash scripts/collect_wrapper.sh   # naver 스킵 (격일 스케줄용)
...
node scripts/harness_runner.mjs --mode=incremental  # incremental 모드 (Day 1+)
node scripts/run_parallel_collect.mjs --exclude-platforms=naver,kbland  # 특정 플랫폼 제외
...
# 베이스라인 메트릭 캡처 (수집 변경 전후 비교용 — Day 0+)
node scripts/verify_collection_baseline.mjs --out=reports/baseline-fullmode.json

# 누락 매물 일회성 회복 (Day 7+ — backlog 처리. 신규 deleted_at 회복은 cron 자동)
node scripts/restore_recollected_listings.mjs           # dry-run
node scripts/restore_recollected_listings.mjs --apply   # 실제 회복
