  ...
  const listingRows = await withDbClient(async (client) => {
    ...
    const rows = await client.query(
      ...
      SELECT * FROM (
        SELECT nl.listing_id, nl.platform_code, nl.source_url,
               ...
               nl.address_text, nl.address_code, nl.room_count, nl.bathroom_count, nl.floor, nl.total_floor, nl.direction, nl.building_use,
               nl.quality_flags, nl.lat, nl.lng,
               nl.sale_price, nl.loan_amount, nl.building_year,
               nl.description_text, nl.monthly_management_cost, nl.walk_time_to_subway, nl.parking_possible,
               nl.features, nl.jibun_address,
               nl.listed_at,
    ...
    const mappedRows = listingRowsInner.map((row) => {
      ...
      return {
        ...
        jibun_address: safeText(row.jibun_address, null),
