...
/**
 * 부동산써브 raw 캡처 결과 -> normalized 변환
 */

import fs from "node:fs";
import path from "node:path";
import { ServeListingAdapter } from "./adapters/serve_listings_adapter.mjs";
...
const inputFile = path.resolve(
  getArg("--input", path.join(process.cwd(), "scripts/serve_raw_samples.jsonl")),
);
const outputFile = path.resolve(
  getArg("--output", path.join(process.cwd(), "scripts/serve_normalized_samples.json")),
);
